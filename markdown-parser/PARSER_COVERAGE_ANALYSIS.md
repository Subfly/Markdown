...
| 优先级 | 特性 | 语法示例 | 说明 |
|--------|------|----------|------|
| **P1** | 图片高级特性 | `![alt](url =200x300)` / `![alt](url){loading=lazy .rounded align=right}` | 扩展图片链接解析逻辑，提取尺寸（`=WxH` 后缀）、属性（loading/class/align），图片节点携带额外属性。解决 Markdown 原生图片无法控制尺寸/样式的痛点 |
